...
if [ "${SKIP_ENV:-false}" = "true" ]; then
  ...
  gcloud run deploy "${SERVICE_NAME}" \
    ...
    --allow-unauthenticated \
    --timeout="300s" \
    --set-env-vars="NODE_ENV=production,LOG_LEVEL=info,MONGODB_DATABASE=testes-mxm-omie,MONGODB_COLLECTION_PAYMENT_INTEGRATIONS=testes-mxm-omie,MXM_BASE_URL=https://insider.mxmcloud.com.br,OMIE_BASE_URL=https://app.omie.com.br,OMIE_ID_CONTA_CORRENTE=6917718230,SYNC_WINDOW_HOURS=26,SLACK_DEVELOPMENT_CHANNEL=C0B97USHTE3,SLACK_ALERT_CHANNEL=C0B9T2KFVM2,GCP_PROJECT_ID=${PROJECT_ID},GCP_REGION=${REGION}" \
